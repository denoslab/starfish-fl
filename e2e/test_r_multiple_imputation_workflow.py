"""
End-to-end test: federated R Multiple Imputation (MICE) workflow.

Uses the same 8-step pattern as test_fl_workflow.py but with
RMultipleImputation model (R mice package) and data containing
missing values.
"""
import re
import time
...
PROJECT_NAME = "e2e-r-mice-project"

TASKS_JSON = (
    '[{"seq": 1, "model": "RMultipleImputation",'
...
def test_r_multiple_imputation_workflow(pages, base_a, base_b, base_c, fixtures_dir):
    """Full federated R MICE workflow through the Controller web portal."""
    ...
    page_a.fill('textarea[name="description"]', "E2E R MICE test")
