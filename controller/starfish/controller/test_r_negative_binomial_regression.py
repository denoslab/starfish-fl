...
from starfish.controller.tasks.r_negative_binomial_regression.task import RNegativeBinomialRegression
...
def make_nb_csv(path, n=200, seed=42):
...
class RNBTestBase(TestCase):
    ...
    def _make_task(self, **kwargs):
        return RNegativeBinomialRegression(make_run(**kwargs))
    ...
    def _setup_dataset(self, run_id=42, n=200, seed=42):
        ...
        make_nb_csv(csv_path, n=n, seed=seed)
        return csv_path


class RNBPrepareDataTest(RNBTestBase):

    @patch.object(RNegativeBinomialRegression, 'is_first_round', return_value=True)
...
class RNBTrainingTest(RNBTestBase):
...
class RNBAggregationTest(RNBTestBase):
    ...
    @patch.object(RNegativeBinomialRegression, 'upload', return_value=True)
    def test_weighted_average_with_equal_se(self, _):
        self._write_mid_artifact(
            'siteA-1-1-mid-artifacts', [0.6, 0.4, -0.2], [0.1, 0.1, 0.1], 100, 0.4)
        self._write_mid_artifact(
            'siteB-1-1-mid-artifacts', [0.4, 0.2, -0.4], [0.1, 0.1, 0.1], 100, 0.6)
        task = self._make_task()
        task.do_aggregate()
