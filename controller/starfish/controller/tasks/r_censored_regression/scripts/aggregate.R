#!/usr/bin/env Rscript
# aggregate.R — Inverse-variance weighted meta-analysis of Tobit coefficients.
#
...
for (art in mid_artifacts) {
  ...
  sigma_weighted   <- sigma_weighted + art$sigma * art$sample_size
  sigma_weight_sum <- sigma_weight_sum + art$sample_size
}
...
result <- list(
  ...
  sigma         = jsonlite::unbox(as.numeric(pooled_sigma)),
