#!/usr/bin/env Rscript
# aggregate.R — Inverse-variance weighted meta-analysis of NB coefficients.
#
...
for (art in mid_artifacts) {
  ...
  alpha_sum     <- alpha_sum + art$alpha * art$sample_size
  alpha_weight  <- alpha_weight + art$sample_size
}
...
pooled_alpha <- alpha_sum / alpha_weight
...
result <- list(
  ...
  alpha         = jsonlite::unbox(pooled_alpha),
